...
export default function Navbar() {
  ...
  return (
    ...
    >
        ...
        <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '70px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <motion.a href="#contact" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
              className="hidden lg:flex btn-solid-cyan"
              style={{ padding: '8px 20px', borderRadius: '8px', fontSize: '13px' }}>
              <Zap size={13} /> Hire Me
            </motion.a>
            <motion.button whileTap={{ scale: 0.85 }} onClick={() => setMenuOpen(o => !o)}
              className="lg:hidden"
              style={{
                width: '38px', height: '38px', borderRadius: '8px',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                background: menuOpen ? 'rgba(0,245,255,0.15)' : 'rgba(0,245,255,0.06)',
                border: '1px solid rgba(0,245,255,0.3)', color: '#00F5FF', cursor: 'pointer',
              }}>
              <AnimatePresence mode="wait" initial={false}>
                {menuOpen
                  ? <motion.span key="x" initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 90, opacity: 0 }} transition={{ duration: 0.16 }}><X size={17} /></motion.span>
                  : <motion.span key="m" initial={{ rotate: 90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: -90, opacity: 0 }} transition={{ duration: 0.16 }}><Menu size={17} /></motion.span>
                }
              </AnimatePresence>
            </motion.button>
          </div>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div key="mobile" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            transition={{ duration: 0.28, ease: [0.22, 1, 0.36, 1] }}
            style={{ overflow: 'hidden', background: 'rgba(5,10,15,0.98)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(0,245,255,0.1)', padding: '0 5%' }}>
              ...
              {navLinks.map((link, i) => {
                ...
                return (
                  <motion.a key={link.label} href={link.href} onClick={() => setMenuOpen(false)}
                    ...
                    <span style={{ fontFamily: 'JetBrains Mono', fontSize: '10px', color: isActive ? '#00F5FF' : 'var(--text-dim)', width: '22px' }}>{link.num}</span>
                    {link.label}
                    {isActive && <div style={{ marginLeft: 'auto', width: '5px', height: '5px', borderRadius: '50%', background: '#00F5FF', boxShadow: '0 0 8px #00F5FF' }} />}
                  </motion.a>
                );
              })}
              <motion.a href="#contact" onClick={() => setMenuOpen(false)}
                initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
                className="btn-solid-cyan" style={{ marginTop: '10px', justifyContent: 'center', padding: '13px' }}>
                <Zap size={14} /> Hire Me
              </motion.a>
            </div>
          </motion.div>
